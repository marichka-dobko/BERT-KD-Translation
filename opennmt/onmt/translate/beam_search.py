...
class BeamSearch(DecodeStrategy):
    ...
    def advance(self, log_probs, attn):
        ...
        torch.div(self.topk_ids, vocab_size, out=self._batch_index.to(torch.float))
        ...
        # self.alive_seq = self.alive_seq.squeeze() #.resize_([30, 5])
        # print(self.alive_seq.shape, self.select_indices.shape)
        # print(self.alive_seq.index_select(0, self.select_indices))
        # exit()
        # print(self.alive_seq.shape, self.topk_ids.view(_B * self.beam_size, 1).shape)
        self.alive_seq = torch.cat(
            # [self.alive_seq.index_select(0, self.select_indices),
            [self.alive_seq,
             self.topk_ids.view(_B * self.beam_size, 1)], -1)
        if self.return_attention or self._cov_pen:
            # print(self.select_indices.shape, attn.shape)
